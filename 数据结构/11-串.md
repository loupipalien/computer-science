...
i 可看作时比对成功的次数, i - j 可看作比对失败次数的上界, 则令 k = 2 * i - j, 即是迭代步数的上届
k = 2 * i + j <= 2(n - 1) - (-1) = 2n - 1 = O(n)
buildNext的时间复杂度是匹配串长度 O(m - 1) = O(m)
即时间复杂度为 O(n) + O(m) = O(n + m)
### KMP 不足
T: 0 0 0 1 0 0 0 0 1
P: 0 0 0 0 1
N: -1 0 1 2 3
只利用了自相似的经验, 未利用不相同的教训
### KMP 算法改进
```
int * buildNext(char * P) {
    int m = (int)strlen(P), j = 0; // 主串指针
